...
```{mermaid}
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '15px', 'fontFamily': 'ui-sans-serif, system-ui, sans-serif', 'primaryColor': '#eaf1fb', 'primaryBorderColor': '#003da5', 'primaryTextColor': '#1f2937', 'lineColor': '#003da5'}}}%%
flowchart TD
    A["Raw texts\n294 narratives"] --> B["Clean and chunk\n750-word passages · 1,543 total"]
    B --> C["Embed with nomic-embed-text\n768 numbers per chunk"]
    C --> D["Reduce dimensions with UMAP\n768 dimensions → 5 dimensions"]
    D --> E["Cluster with HDBSCAN\n59 topics found in the sample"]
    E --> F["Find topic words with c-TF-IDF\nmost distinctive words per topic"]
    F --> G["Generate labels with llama3.1\nhuman-readable topic names"]
    G --> H["Results\nCSV files and interactive charts"]
```
...
All charts are interactive — hover over bars and cells to see exact values, and click legend items to show or hide topics. Each chart can also be downloaded as a standalone HTML file from the [GitHub repository](https://github.com/jinghanlib/topic-modeling-slave-narratives) under `outputs/bertopic_sample_nomic_sensitive_lemmatized/`. Open any `.html` file directly in a browser to explore it.
...
This chart shows the same topic-by-decade data as individual line charts, one panel per topic. It makes it easier to see the shape of a single topic's trend across time without other topics in the way.
...
Use this alongside the grouped bar chart to compare how individual topics rise or fall across decades.
